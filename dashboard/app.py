...
COOLDOWN_FILE = os.environ.get("COOLDOWN_FILE", "/root/.hermes/curator-pool/cooldown_status.json")
CB_STATE_FILE = os.environ.get("CB_STATE_FILE", "/root/.hermes/curator-pool/circuit_state.json")
CB_STATE_LOCK = os.environ.get("CB_STATE_LOCK", "/root/.hermes/curator-pool/circuit_state.lock")
...
def load_candidates():
    ...
    ~/.hermes/curator-pool/candidate_providers.json wins over repo default."""
    paths = [
        "/root/.hermes/curator-pool/candidate_providers.json",
...
def load_excluded_models():
    ...
    ~/.hermes/curator-pool/excluded_models.json wins; falls back to repo
    default mounted in the image."""
    paths = [
        "/root/.hermes/curator-pool/excluded_models.json",
...
def load_curator_recommendations():
    ...
    markdown report to ~/.hermes/operator/discoveries/<date>.md, not JSON).
    This endpoint returns empty if the legacy recommendations.json is absent
    — UI handles gracefully."""
    path = "/root/.hermes/curator-pool/recommendations.json"
    try:
        with open(path) as f:
            d = json.load(f)
        ...
        return {"best": [], "code": [], "mid": [], "fast": [], "note": "no recommendations.json (op-discover writes markdown to ~/.hermes/operator/discoveries/ instead)"}
    except Exception as e:
...
def load_curator_history(n: int = 30):
    ...
    events = []
    paths = [
        ("/root/.hermes/curator-pool/circuit_history.jsonl", "breaker"),
        ("/root/.hermes/curator-pool/rebalance_history.jsonl", "rebalance"),
    ]
